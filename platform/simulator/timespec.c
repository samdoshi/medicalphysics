...
struct timespec timespec_from_double(double s) {
    struct timespec ts = {
        .tv_sec = (long)s,
        .tv_nsec = (long)((s - (long)(s)) * NSEC_PER_SEC),
    };
...
#define TEST_ADD(ts1_sec, ts1_nsec, ts2_sec, ts2_nsec, expect_sec,        \
                 expect_nsec)                                             \
    {                                                                     \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {     \
            printf(                                                       \
                "timespec_add({%ld, %ld}, {%ld, %ld}) returned wrong "    \
                "values\n",                                               \
...
#define TEST_SUB(ts1_sec, ts1_nsec, ts2_sec, ts2_nsec, expect_sec,        \
                 expect_nsec)                                             \
    {                                                                     \
        struct timespec ts1 = { .tv_sec = ts1_sec, .tv_nsec = ts1_nsec }; \
        struct timespec ts2 = { .tv_sec = ts2_sec, .tv_nsec = ts2_nsec }; \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {     \
            printf(                                                       \
                "timespec_sub({%ld, %ld}, {%ld, %ld}) returned wrong "    \
                "values\n",                                               \
                (long)(ts1_sec), (long)(ts1_nsec), (long)(ts2_sec),       \
                (long)(ts2_nsec));                                        \
            printf("    Expected: {%ld, %ld}\n", (long)(expect_sec),      \
...
#define TEST_MOD(ts1_sec, ts1_nsec, ts2_sec, ts2_nsec, expect_sec,        \
                 expect_nsec)                                             \
    {                                                                     \
        struct timespec ts1 = { .tv_sec = ts1_sec, .tv_nsec = ts1_nsec }; \
        struct timespec ts2 = { .tv_sec = ts2_sec, .tv_nsec = ts2_nsec }; \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {     \
            printf(                                                       \
                "timespec_mod({%ld, %ld}, {%ld, %ld}) returned wrong "    \
                "values\n",                                               \
                (long)(ts1_sec), (long)(ts1_nsec), (long)(ts2_sec),       \
                (long)(ts2_nsec));                                        \
            printf("    Expected: {%ld, %ld}\n", (long)(expect_sec),      \
    ...
    {                                                                      \
        ...
        if (func(ts1, ts2) != expect) {                                    \
            printf(#func "({%ld, %ld}, {%ld, %ld}) returned %s\n",         \
                   (long)(ts1_sec), (long)(ts1_nsec), (long)(ts2_sec),     \
                   (long)(ts2_nsec), (expect ? "FALSE" : "TRUE"));         \
            result = 1;                                                    \
        }                                                                  \
    }
...
#define TEST_FROM_TIMEVAL(in_sec, in_usec, expect_sec, expect_nsec)          \
    {                                                                        \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {        \
            printf(                                                          \
                "timespec_from_timeval({%ld, %ld}) returned wrong values\n", \
